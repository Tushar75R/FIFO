import { IPv4Packet, IPv6Packet } from "./ProtocolTypes";
import chalk from "chalk";
import EventEmitter from "events";

class FIFOQueue extends EventEmitter {
  private queue: (IPv4Packet | IPv6Packet)[] = [];
  private readonly maxSize: number;
  private congestionThreshold: number;
  private qosPriorityMap: Map<string, number>;

  constructor(
    maxSize: number,
    congestionThreshold: number,
    qosPriorityMap: Map<string, number>
  ) {
    super();
  ...
  enqueue(packet: IPv4Packet | IPv6Packet): void {
    if (this.queue.length >= this.maxSize) {
      this.emit("packetDrop", packet, "QueueOverflow");
    ...
    if (this.queue.length >= this.congestionThreshold) {
      this.emit("packetDrop", packet, "Congestion");
      console.log(
        chalk.yellow(`Congestion detected. Dropping low priority packets.`)
      );
  ...
  dequeue(): IPv4Packet | IPv6Packet | undefined {
    const packet = this.queue.shift();
    if (packet) {
      this.emit("packetTransmit", packet);
      console.log(
        chalk.green(
          `Packet transmitted successfully: ${this.formatPacket(packet)}`
        )
      );
    }
    return packet;
  ...
  setCongestionThreshold(newThreshold: number): void {
    this.congestionThreshold = newThreshold;
    this.emit("congestionThresholdChange", newThreshold);
    console.log(chalk.blue(`Congestion threshold updated to: ${newThreshold}`));
  }

  private formatPacket(packet: IPv4Packet | IPv6Packet): string {
    if (packet.version === 4) {
      const ipv4Packet = packet as IPv4Packet;
      return `
      ...
      const ipv6Packet = packet as IPv6Packet;
      return `
  ...
  private getProtocol(packet: IPv4Packet | IPv6Packet): string {
